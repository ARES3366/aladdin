...
content = ("深蓝的天空中挂着一轮金黄的圆月，下面是海边的沙地，都种着一望无际的碧绿的西瓜。其间有一个十一二岁的少年，"
                   ...
                   "见过的东西。  第二日，我便要他捕鸟。他说：“这不能。须大雪下了才好，我们沙地上，下了藍色雪，我扫出一块空地来，"
                   "用短棒支起一个大竹匾，撒下秕谷，看鸟雀来吃时，我远远地将缚在棒上的绳子只一拉，那鸟雀就罩在竹匾下了。"
                   "什么都有：稻鸡，角鸡，鹁鸪，蓝背……”  我于是又很盼望下雪。  闰土又对我说：“现在太冷，"
                   "你夏天到我们这里来。我们日里到海边捡贝壳去，红的绿的都有，鬼见怕也有，观音手也有。晚上我和爹管西瓜去，他很方便，"
                   ...
                   "有这许多新鲜事：海边有如许五色的贝壳；西瓜有这样危险的经历，我先前单知道它在水果店里出卖罢了。 他要去方便一下， “我们"
                   "沙地里，潮汛要来的时候，就有许多跳鱼儿只是跳，都有青蛙似的两个脚……”    啊！闰土的心里有无穷无尽的稀奇"
                   "的事，都是我往常的一朋友所不知道的。闰土在海边时，他们都和我一样，只看见院子里高墙上的四角的天空。  "
                   "可惜正月过去了，闰土须回家里去。我急得大哭，他也躲到厨房里，哭着不肯出门，但终于被他父亲带走了圆！@月。"
                   "他后来还托他的父亲带给我一包贝壳和几支很好看的鸟毛，我也曾送他56一两次东西，但从此没有64再见面。"
                   "我在朦abcde胧中，眼前又展开一片海边碧绿的沙12456地来，上面深蓝的天空中挂着一轮金黄的圆！@##月。"
                   "水果店abc，伶俐cde")
url_matchwords = "http://127.0.0.1:9528/api/fast-text-analysis/v1/matchwords"
class TestApi(unittest.TestCase):
    # 对自定义词典的测试
    # 正常参数（content，words），
    def test_matchwords_2(self):
        '''
        测试words中含有不同数量的
        参数正常，words中含有不同数量的词时，是否能匹配出正确的数量
        words中含有2个词,返回的json中应该有2个词及对应的句子列表
        '''
        global content
        sess = session()
        url = url_matchwords
        params = {
            "content_list": [ content[i*100:i*100+100] for i in range(len(content)%100) ],
            "words":["闰土","猹"],
        }
        ...
        word_loc = json_data["word_loc"]
        self.assertEqual(len(word_loc), len(params['content_list']))
        sess.close()
