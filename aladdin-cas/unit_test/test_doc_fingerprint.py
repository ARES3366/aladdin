...
class TestDocFingerprint(unittest.TestCase):
    def test_dfp1(self):
        ...
        fingerprint =s.keywords(content)
        print(fingerprint)
        self.assertTrue(isinstance(fingerprint,list))
    
    def test_dfp(self):
        global s_content
        content = s_content
        print('============content length is %s'%len(content))
        fingerprint =s.simhash(content)
        print(fingerprint)
        self.assertTrue(isinstance(fingerprint,str))
    def test_dfp_nopos(self):
        global s_content
        content = s_content
        print('============content length is %s'%len(content))
        fingerprint =s.simhash_noPOS(content)
        print(fingerprint)
        self.assertTrue(isinstance(fingerprint,str))
    def test_dfp22(self):
    ...
    def test_dfp_short_text(self):
        short_text = '我们人民中国中华人民共和国。人民。中国。'
        dfp = s.simhash(short_text)
        print(dfp)
    def test_word_to_hash(self):
        word="中国"
        hashweight=s.word_to_hash(word)
        print(hashweight)
        self.assertTrue(isinstance(hashweight,str))
    def test_word_to_hash2(self):
        word="=="
        hashweight=s.word_to_hash(word)
        print(hashweight)
    def test_big_content(self):
        global s_content
        content=s_content * int(3* 1024 / (len(s_content)))
        fp = s.simhash(content)
        print(fp)
