...
# from image_analysis.do_face_handler import aiomilvus, doFaceMetaDataPost, doFaceMetaDataGet, doFaceMetaDataPut, doFaceMetaDataDelete, doSimilarFaceSearch 
from fta.do_fta import fastTextAnalysis
...
~                                                                                                                                         
'''
url_fta = "http://127.0.0.1:9528/api/fast-text-analysis/v1/analysis"
url_keyw ="http://127.0.0.1:9528/api/fast-text-analysis/v1/fingerprint_keywords"
class TestApi(unittest.TestCase):
    def test_1fta(self):
        ...
        sess = session()
        s_get = sess.post(url_fta, json=params)
    ...
    def test_2_4fta(self):
        ...
        sess.close()
    def test_2_5fta(self):
        #参数正常，返回数据正常
        params={}
        params['content']=s_content4
        action={}
        action['common_class']=True
        action['keywords']={"top_num":10,"keyphrase":False}
        action['abstract']=False
        action['fingerprint']=False
        params['action']=action
        sess = session()
    ...
    def test_4fta(self):
        ...
        params['content']=s_content4
        action={}
        action['keywords']={"top_num":101,"keyphrase":True}
        action['common_class']=123
        action['abstract']=True
        action['fingerprint']=True
        params['action']=action
        sess = session()
    ...
    def test_5fta(self):     
        ...
        params['content']=s_content4
        action={}
        action['keywords']={"top_num":10,"keyphrase":123}
        action['common_class']=True
        action['abstract']=True
        action['fingerprint']=True
        params['action']=action
    ...
    def test_10fta(self):
        ...
        self.assertTrue(isinstance(data, str))
        sess.close()
    def test_2keywords(self):
        #参数正常，返回数据正常
        params={}
        params['content']=s_content1
        
        sess = session()
        s_get = sess.post(url_keyw, json=params)
        json_data = json.loads(s_get.content.decode('utf8'))
        print("-------------- 2------------------",json_data)
        status = json_data['status']
        self.assertEqual(status,0)
        data = json_data["keywords"]
